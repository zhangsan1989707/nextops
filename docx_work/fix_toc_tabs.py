...
toc_entries = {
    "3.1 主要职责分工10": ("3.1 主要职责分工", "10"),
    "3.1.1 蓉政通二期分工10": ("3.1.1 蓉政通二期分工", "10"),
    "3.1.2 项目管理（联合）10": ("3.1.2 项目管理（联合）", "10"),
    "3.1.3 项目运营（数城）10": ("3.1.3 项目运营（数城）", "10"),
    "3.1.4 项目运维（中天）11": ("3.1.4 项目运维（中天）", "11"),
}
...
with TemporaryDirectory() as tmp:
    ...
    for p in root.xpath("//w:p", namespaces=NS):
        ...
        if joined in toc_entries and texts:
            label, page = toc_entries[joined]
            texts[0].text = label
            for t in texts[1:-1]:
                t.text = ""
            texts[-1].text = page
